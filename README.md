# FreeList — 슬롯 재사용 자료구조를 밑바닥부터

> 빈 칸 목록을 위한 별도 자료구조 없이, **빈 칸 자체에 "다음 빈 칸 번호"를 적어** 관리한다. C#으로 두 가지를 만들었다.
>
> - **`SlotMap<T>`** — 고정 크기 슬롯 + 세대 핸들 (변형 A)
> - **`BlockAllocator`** — 가변 크기 블록. 경계 태그·병합·명시적 free 리스트 (변형 B)
>
> 출발점은 Unity DI 라이브러리 VContainer의 내부였다 ([9절](#9-실제-구현과-비교--vcontainer의-freelistt)).
...
|---|---|---|
| 식별자 | `SlotHandle { Index, Generation }` | `int offset` (생짜 위치) |
| 이중 해제 | ✅ 세대로 거부 | ✅ 헤더의 빈 플래그로 거부 (단, 아래 주의) |
| 재사용된 자리에 옛 식별자 | ✅ 세대 불일치로 거부 | ❌ **못 막는다** |
| 범위 밖 / 정렬 안 맞는 위치 | ✅ 거부 | ✅ 거부 |
| 블록 한가운데를 가리키는 위치 | 해당 없음 | ❌ **못 막는다** |

`BlockAllocator`의 두 ❌는 버그가 아니라 **API가 정의한 계약**이다.

```csharp
alloc.TryAlloc(64, out int p);
alloc.Free(p);
alloc.TryAlloc(64, out int q);   // 같은 자리를 재사용 → q == p

alloc.Free(p);                   // p와 q가 같은 숫자다. 구분할 정보가 없다
alloc.AsSpan(p);                 // 위와 같음
```

`int offset` 하나로는 "몇 번째 발급분인가"를 표현할 수 없다. **C의 `free(ptr)`가 use-after-free를 못 막는 것과 정확히 같은 이유**고, 교과서적 할당기를 만드는 것이 목적이라 그대로 뒀다.

블록 한가운데를 가리키는 위치도 마찬가지다. `Free(offset)`은 `offset - 4`를 헤더로 읽는데, 운 나쁘게 그 자리의 payload 바이트가 검증(크기 ≥ `MinBlock`, 범위 안, 정렬)을 다 통과하면 아레나가 망가진다. 확률은 낮지만 **구조적으로 못 막는다.**

막고 싶다면 방법은 하나다 — 헤더에 세대 필드를 두고 `int offset` 대신 `{offset, generation}` 핸들을 반환하는 것. `SlotMap`에서 한 것과 같다. 그러면 블록당 4바이트를 더 내고, 오버헤드가 8에서 12바이트가 된다. **안전성과 공간의 맞교환**이고, 이 프로젝트는 후자를 골랐다.
...
칸 크기가 고정이 아니면 "3번 칸"이라는 번호를 못 매긴다. 그래서 **관리 정보를 옆 배열이 아니라 데이터 사이에 심는다.**
...
헤더/푸터 = 블록 전체 크기 | (비었으면 1)
            크기가 4의 배수라 최하위 비트가 항상 비어 있다
```
...
`_freeHead`에서 `next`를 따라가면 **빈 블록만** 훑는다. SlotMap의 `_freeHead` + `_next[]`를 별도 배열 없이 재현한 것이다.
...
| 시그니처 | 설명 |
|---|---|
| `BlockAllocator(int capacity)` | 아레나 바이트 수. 4의 배수이고 `MinBlock`(16) 이상이어야 한다 |
| `bool TryAlloc(int size, out int offset)` | **바이트 수**를 요청하고 위치를 받는다. 값이 아니라 자리를 준다 |
| `bool Free(int offset)` | 반납. 양옆이 비었으면 자동으로 합친다 |
| `Span<byte> AsSpan(int offset)` | 그 블록의 payload를 여는 창문 |
| `AllocatorStats GetStats()` | 아레나를 걸으며 진단값 5개를 센다 |
| `FitPolicy Policy { get; set; }` | `First`(기본) / `Best` — 빈 블록 선택 정책 |

`SlotMap`은 **값을 보관해주는 그릇**이고 `BlockAllocator`는 **자리를 나눠주는 관리인**이다. 후자는 안에 뭐가 들어가는지 모른다.

**불변식** — `UsedBytes + FreeBytes + OverheadBytes == Capacity`. `GetStats()`가 아레나를 물리적으로 걸으며 이걸 검증하므로, free 리스트가 현실과 어긋나면 즉시 드러난다. (실제로 3단계 버그를 이 가드가 잡았다.)

### 테스트

xUnit 테스트 **74개** (`SlotMap` 39 + `BlockAllocator` 35). 모든 테스트는 실제로 겪은 버그 하나씩을 붙잡고 있다.
...
병합 그룹 5개는 **1단계 구현에서 의도적으로 빨간불이었다.** 경계 태그를 넣기 전까지 정확히 그 다섯만 실패하는 것이 진도의 척도였다.
...
### 문제 9: 벽시계 시간은 무엇을 재고 있는지 말해주지 않는다

**상황**: 순회 비용이 밀집도에 따라 어떻게 변하는지 재려 했다. 예상은 "총 시간은 밀집도와 무관하게 일정하고, 원소당 비용만 반비례로 치솟는다"였다. 100만 칸을 훑는 건 밀집도와 상관없으니까.

`Stopwatch`로 잰 숫자는 자료구조의 성능이 아니라 **그 순간 CPU가 실행하던 기계어의 성능**이다. 아래 세 단계를 거치고서야 둘이 분리됐다.
...
### 문제 10: 비율 지표의 분자와 분모가 다른 사건을 세고 있었다

가변 크기 할당기(변형 B)에서 first-fit과 best-fit 중 무엇을 쓸지 정해야 했다. 시간이 환경에 흔들린다는 걸 문제 9에서 확인했으므로 **탐색 횟수**를 세기로 했다 — 계층형 컴파일도 GC도 Debug 빌드도 카운터는 못 바꾼다.
...
문제 9의 세 사례는 **측정 환경**(Debug 빌드, JIT 워밍업, 계층형 컴파일)이 숫자를 흔든 경우였다. 이번은 다르다 — 환경은 정확했고 **지표의 정의 자체가 틀렸다.** 도구를 바꿔도 안 잡히는 종류다.

> **원칙** — 비율을 정의할 때 분자와 분모가 같은 사건 집합을 세고 있는지 확인한다.

### 문제 11: 3단계는 136배 빨라지면서 단편화를 두 배로 악화시켰다

빈 블록의 payload에 링크를 심어 명시적 free 리스트를 만들자(3단계) 탐색이 극적으로 줄었다.
...
여기까지는 예상대로였다. 그런데 단편화율을 같이 보니 반대 방향으로 움직였다.
...
원인은 **LIFO**다. 아레나를 걷는 방식은 항상 주소가 가장 낮은 적합 블록을 고르므로 할당이 앞쪽에 뭉치고 뒤쪽에 큰 연속 공간이 남는다. 반면 free 리스트는 최근 해제된 블록을 머리에 꽂으니 주소 순서를 완전히 잃고, 흩어진 조각을 순서 없이 나눠주다 보니 조각이 계속 조각으로 남는다.

**속도를 136배 얻으면서 공간 효율을 잃은 것이다.** 재보기 전에는 3단계가 순수한 개선인 줄 알았다.

여기서 다음 개선 방향이 나왔다 — 리스트를 주소 오름차순으로 유지하면 양쪽을 다 가질 수 있다. 대가는 `PushFree`가 O(1)에서 O(빈 블록 수)로 오르는 것인데, `Free`가 `TryAlloc`보다 드물다면 남는 장사다. 이것도 재보면 된다.

### 문제 12: 탐색 38배 차이가 시간 6배 차이로 나온 이유
...
**빌드 산출물이 소스보다 오래됐는지 먼저 확인한다.** 코드를 아무리 들여다봐도 안 보이는 게 당연하다 — 보고 있는 코드와 돌고 있는 코드가 다르니까.
...
`BlockAllocator`는 모든 것이 `int`다. 크기도, 위치도, 링크도, 플래그가 섞인 원본도. **컴파일러가 이들을 구분해주지 않으므로 이름이 유일한 방어선이다.**

특히 두 값이 항상 짝으로 다닌다.

```
블록 전체 크기   걷기 · 검사 · 헤더/푸터에 기록할 값
...
| 이름 | 담는 것 | 쓰는 곳 |
|---|---|---|
| `blockSize` | 헤더/푸터 포함 전체 | 걷기(`pos += blockSize`), 범위 검사, `WriteBlock` |
| `length` | payload만 | `AsSpan`이 반환할 창문 길이 |
| `raw` | 크기 + 빈 플래그가 섞인 원본 | `ReadSize`/`IsFree`가 각각 분해해 쓴다 |
| `blockStart` | 헤더가 있는 위치 | 모든 헤더 접근의 기준점 |
| `offset` | payload가 시작하는 위치 | 공개 API가 주고받는 값. `blockStart + HeaderSize` |

`raw`를 `size`라고 부르면 플래그가 붙은 값을 크기로 쓰게 되고, `blockSize`를 `length`라고 부르면 스팬이 이웃 블록을 침범한다. `blockStart`와 `offset`이 4바이트 차이라는 것도 이름으로만 구분된다.

> **원칙** — 같은 타입이지만 다른 의미인 값에는 다른 이름을 준다. 타입 시스템이 안 막아주는 자리일수록 이름이 계약이 된다.

`SlotMap`에서 `SlotHandle`을 `readonly struct`로 만든 것도 같은 이유다. `Remove(int, int)`를 노출했을 때는 인덱스와 세대를 바꿔 넣어도 컴파일이 됐지만, 핸들 하나로 묶은 뒤에는 그럴 수가 없다. **이름으로 구분하는 것보다 타입으로 구분하는 쪽이 강하다.**
...
- [ ] **가변 크기 블록 할당기 (BlockAllocator)** — 진행 중. `byte[]` 아레나를 `[헤더|payload]` 블록으로 나눠 쓰고, 헤더 `int` 하나에 블록 크기와 빈 플래그를 겹쳐 담는다 (크기가 4의 배수라 최하위 비트가 늘 비어 있다)
  ...
  - [x] 2단계 — 경계 태그(푸터)와 병합. 블록 끝에 크기를 한 번 더 적어 앞 블록 시작을 O(1)로 역산한다. 대가는 블록당 오버헤드 4→8바이트. 테스트 39/39 통과
  - [x] 3단계 — 빈 블록의 payload에 prev/next를 심어 명시적 free 리스트를 만들었다. 탐색이 전체 블록이 아니라 빈 블록만 훑는다. SlotMap의 `_freeHead`/`_next`를 별도 배열 없이 재현한 셈. 대가는 `MinPayload` 4→8
  - [x] 4단계 — 계측(탐색 횟수·단편화율)을 붙이고 first-fit vs best-fit을 측정. 워크로드에 따라 승자가 갈린다는 결론 (문제 10~13 참고)
  - [ ] 5단계 — 주소 순서 free 리스트. LIFO가 만든 단편화(문제 11)를 되돌리면서 리스트의 탐색 이득은 유지할 수 있는지
  - [ ] 6단계 — 크기 클래스(segregated list). 자주 쓰는 크기를 따로 관리하면 탐색과 단편화가 어떻게 달라지는지
...
이건 `SlotMap`에서 측정한 밀집도 문제와 **정확히 같은 것**이다 (6절 문제 9). 용량 100만에 살아있는 게 1000개면 100만 번 훑는다. VContainer도 같은 구조인데 n이 수십 개라 비용이 안 되는 것뿐이다.
