...
>
> 출발점은 Unity DI 라이브러리 VContainer의 내부였다 ([2절](#2-출발점--vcontainer의-freelistt와-무엇이-다른가)).
...
Unity DI 라이브러리 **VContainer**의 내부를 읽다가 시작됐다. 진입점을 매 프레임 돌리는 `PlayerLoopRunner`가 목록을 `List<T>`가 아니라 `FreeList<T>`로 들고 있었고, "왜 굳이?"가 궁금했다. 직접 만들어보기로 했다. (비교는 [2절](#2-출발점--vcontainer의-freelistt와-무엇이-다른가) 참고 — 만들고 나서 다시 읽으니 **같은 이름인데 다른 물건**이었다.)
...
## 2. 출발점 — VContainer의 `FreeList<T>`와 무엇이 다른가
...
### 문제 2: 벽시계 시간은 무엇을 재고 있는지 말해주지 않는다
...
### 문제 3: 속도 136배를 얻으면서 단편화를 두 배로 잃었다
...
> 나머지 열 건은 [`docs/디버깅-기록.md`](docs/디버깅-기록.md)에 있다. 체인 초기화, 방어 코드가 방어를 못 한 경우, 우연히 맞던 확장 로직, 무한 루프로 드러난 테스트 실패, 지표 정의 오류 등.
...
- [ ] **dense/sparse 분리** — 보류. 아래 셋 중 하나가 성립하면 재검토한다 (근거와 비용 모델은 문제 2 참고)
...
- [ ] **가변 크기 블록 할당기 (BlockAllocator)** — 진행 중. `byte[]` 아레나를 `[헤더|payload]` 블록으로 나눠 쓰고, 헤더 `int` 하나에 블록 크기와 빈 플래그를 겹쳐 담는다 (크기가 4의 배수라 최하위 비트가 늘 비어 있다)
  ...
  - [x] 4단계 — 계측(탐색 횟수·단편화율)을 붙이고 first-fit vs best-fit을 측정. 워크로드에 따라 승자가 갈린다는 결론 ([`docs/디버깅-기록.md`](docs/디버깅-기록.md) 참고)
  - [ ] 5단계 — 주소 순서 free 리스트. LIFO가 만든 단편화(문제 3)를 되돌리면서 리스트의 탐색 이득은 유지할 수 있는지
...
- [VContainer — hadashiA](https://github.com/hadashiA/VContainer) — 이 프로젝트의 출발점. `Runtime/Internal/FreeList.cs`와 `Runtime/Unity/PlayerLoopRunner.cs` (2절 비교 참고)
