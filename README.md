...
### 문제 10: 이번엔 내가 정의한 지표가 거짓말을 했다

가변 크기 할당기(변형 B)에서 first-fit과 best-fit 중 무엇을 쓸지 정해야 했다. 시간은 이미 세 번 속았으니(문제 9) 이번엔 **탐색 횟수**를 세기로 했다 — 계층형 컴파일도 GC도 Debug 빌드도 카운터는 못 바꾸니까.
...
측정이 거짓말을 한 게 이 프로젝트에서 네 번째다. 앞의 셋은 도구(Debug 빌드, JIT, 계층형 컴파일)가 범인이었지만, **이번엔 내가 정의한 지표 자체가 범인이었다.**
...
### 한 변수에 두 의미를 담으면 다음 줄이 틀린다

이 프로젝트에서 같은 모양의 버그가 **다섯 번** 났다.

| 변수 | 이름이 말하는 것 | 실제로 담긴 것 | 터진 곳 |
|---|---|---|---|
| `ReadSize`의 `size` | 크기 | 크기 + 플래그 원본 | `IsFree`가 항상 `false` |
| `sizeWithFooter` | 크기 | 위치 | 헤더를 아예 안 씀 |
| `ReadPrev()` 반환값 | 링크 값 | 링크의 위치 | `WritePrev`가 아무 데나 씀 |
| `AsSpan`의 `size` | payload 길이 | 블록 전체 크기 | 스팬이 이웃 헤더를 침범 |
| `AsSpan`의 `size` | 블록 전체 크기 | payload 길이 | 검사가 무너져 작은 블록을 전부 거부 |

**전부 같은 실패다.** 이름을 믿고 쓴 *다음 줄*이 틀렸다. 컴파일러는 둘 다 `int`라 아무 말도 안 해준다.

이 코드에는 항상 두 값이 짝으로 다닌다.

```
블록 전체 크기   걷기 · 검사 · 헤더/푸터에 쓸 값
payload 길이     사용자에게 줄 값
                 둘의 차이가 정확히 BlockOverhead
```

`blockSize`와 `length`로 **이름을 갈라두면 이 사고가 구조적으로 안 난다.** 마지막 두 줄은 같은 메서드 안에서 같은 이름이 반대 의미를 오간 경우다.

> **교훈** — 변수 이름은 "무엇인지"를 말해야 한다. 이름과 실제가 어긋나면, 며칠 뒤의 자기 자신이 반드시 그 이름을 믿고 쓴다.
